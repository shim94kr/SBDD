...
class SplitResMLP(torch.nn.Module):
    def __init__(self, net_x=False, n_cond=-1):
        super().__init__()
        self.net_x = net_x
        self.embedding = torch.nn.Embedding(n_cond, 16)
        self.network_t = ResMLP(16, 1, 256, 3, n_cond=-1)
        if self.net_x:
            self.network_x = ResMLP(2, 2, 256, 5, n_cond=-1)
    
    def forward(self, x, t):
        tw = self.network_t(self.embedding(t), None)
        tw = torch.sigmoid(tw)
        xw = self.network_x(x, None) if self.net_x else x
        return tw * xw


class MLP(torch.nn.Module):
    def __init__(self, input_size, output_size=None, hidden_size=256, num_hidden_layers=5, activation=torch.nn.ReLU):
        super().__init__()
...
if __name__ == "__main__":
    ...
    # torch int noisy labels
    noise_labels = torch.randint(0, 1000, (16,)).cuda()
    
    y = model(x, noise_labels, class_labels=None)
    print(y.shape) # torch.Size([16, 4, 32, 32])